...
<body>
	<div class="container">
		...
		<<h2 class =" heading">Print Employee</h2>
...
$SQL = "SELECT * FROM tb_employee WHERE E_ID LIKE '$term'";
  $result = mysqli_query($connection, $SQL);
  if ($result->num_rows > 0) {
     echo "<table><tr><th>ID</th><th>Name</th><th>Assigned_Table</th><th>Designation</th></tr>";
     // output data of each row
     while($row = mysqli_fetch_array($result)) {
         echo "<tr><td>".$row["E_ID"]."</td><td>".$row["E_Name"]."</td><td>".$row["Assigned_Table"]."</td><td>".$row["Designation"]."</td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
