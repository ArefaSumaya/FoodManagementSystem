...
<center><h2 class="heading"><br> View By Customer Information</h2></center>
        <div style="position:absolute;margin-top:50px;margin-left:10px;" id="print">
        <?php
...
if ($result->num_rows > 0) {
     echo "<table><tr><th>ID</th><th>Name</th><th>Contact_Number</th><th class='dontprint'>Action</th><th class='dontprint'> Action</th><th class='dontprint'>Action</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>".$row["C_ID"]."</td><td>".$row["C_Name"]."</td><td>". $row["Contact_Number"]."</td><td class='dontprint'><a href='edit_customer.php?ID=".$row['C_ID']."' style='text-decoration: none' />Edit</a></td><td class='dontprint'><a href='delete_customer.php?ID=".$row['C_ID']."' style='text-decoration: none;' />Delete</a></td><td class='dontprint'><a href='print_customer.php?ID=".$row['C_ID']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
